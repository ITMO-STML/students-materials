...
def process_audio_and_extract_embs_no_av(file_name, content_encoder, device):
    file = file_name
...
def process_directory_no_av(input_dir, output_dir, content_encoder, device):
    """
    ...
    for root, dirs, files in tqdm(os.walk(input_dir)):
        for file in files:
            ...
            if file.endswith(".seg_B2"):  # Ищем только WAV-файлы
                ...
                ph_embs = process_audio_and_extract_embs_no_av(file_name, content_encoder, device)
                keys = []
                values = []
