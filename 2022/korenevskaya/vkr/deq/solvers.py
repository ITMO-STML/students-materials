...
def scalar_search_armijo(phi, phi0, derphi0, c1=1e-4, alpha0=1, amin=0):
            #objective func, start, gradient, (assumptions by nastya)
    ite = 0
...
def line_search(update, x0, g0, g, nstep=0, on=True):
        # direction, start, g(x0), objective (assumption by nastya) 
    """
...
def broyden(f, x0, threshold, eps=1e-5, stop_mode="rel", ls=False, name="unknown"):
    bsz, total_hsize, seq_len = x0.size() # batch, features, time
    g = lambda y: f(y) - y # serch for zeros of g
    dev = x0.device
    alternative_mode = 'rel' if stop_mode == 'abs' else 'abs'
    
    x_est = x0           # (bsz, 2d, L') # initialization of z. In fact, always start with x0 = 0
    gx = g(x_est)        # (bsz, 2d, L') # residual 
    nstep = 0
    tnstep = 0
    ...
    while nstep < threshold:
        ...
        nstep += 1                                  #direction, start, g(start), objective function 
        tnstep += (ite+1)
    ...
    return lowest_xest, lowest_gx


def anderson3stable(f, x0, m, max_iter, lam=5*1e-5, tol=1e-6, beta = 1.0):
    """ Anderson3 acceleration for fixed point iteration. 
        x must be 3-dimensional, so compatible with wav2vec.
    """
    bsz, d, W = x0.shape
    X = torch.zeros(bsz, m, d*W, dtype=x0.dtype, device=x0.device)
    F = torch.zeros(bsz, m, d*W, dtype=x0.dtype, device=x0.device)
    ...
    for k in range(2, max_iter):
        ...
        if (res[-1] < tol):
            break  
    return X[:,k%m].view_as(x0), res[-1]


def anderson(f, x0, m, max_iter, lam=1e-5, tol=1e-3, beta = 1.0):
    """ Anderson acceleration for fixed point iteration. 
        x must be 4-dimensional: bsz x channels x features x time. So, compatible with ResNets.
    """
    bsz, d, H, W = x0.shape
