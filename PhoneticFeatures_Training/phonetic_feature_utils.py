...
def compute_balanced_accuracy(outputs, targets, active_heads):
    ...
    overall_balanced_acc = sum(per_feature_balanced_acc.values()) / len(per_feature_balanced_acc)
...
def evaluate_before_training(model, loader, device, active_heads, criterion_dict):
    loss, acc, feature_acc = evaluate(model, loader, device, active_heads, criterion_dict)
    print("\nBaseline before training:")
    print(f"Loss: {loss:.4f}")
    print(f"Accuracy: {acc:.4f}")
    for feature_idx, value in feature_acc.items():
        print(f"Feature {feature_idx} acc: {value:.4f}")
