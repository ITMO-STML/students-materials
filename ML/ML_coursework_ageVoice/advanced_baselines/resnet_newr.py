...
# Оптимизированныя ReSNet (улучшенная версия)
def build_optimized_resnet(input_dim=1024, dropout_rate=0.4, l2_reg=0.0003):
    """
    Оптимизированная ResNet для максимального F1:
    - Расширение до 1024 (сохраняем размерность)
    - 4 residual блока с плавным уменьшением
...
# Обучение optimized ReSNet
def train_optimized_resnet(X, y):
    print("\nOptimized ReSNet (улучшенная архитектура)")
    ...
    model = build_optimized_resnet(input_dim=WAVLM_DIM, dropout_rate=0.4, l2_reg=0.0003)
...
def train_improved_resnet(X, y):
    ...
    inputs = layers.Input(shape=(WAVLM_DIM,))
    x = layers.Dense(512, kernel_regularizer=reg)(inputs)
    x = layers.BatchNormalization()(x)
    x = layers.Activation('relu')(x)
    x = layers.Dropout(0.4)(x)
    
    x = improved_residual_block(x, 256, 0.4, reg)
    x = improved_residual_block(x, 128, 0.4, reg)
    x = improved_residual_block(x, 64, 0.4, reg)
    x = improved_residual_block(x, 32, 0.4, reg)
...
if __name__ == "__main__":
    print("\nКлассификация возраста (Эмбеддинги)")
    ...
    # Baseline MLP
    results['Baseline MLP'] = train_baseline_mlp(X, y)
    
    # Improved ResNet (старая версия)
    results['Improved ResNet'] = train_improved_resnet(X, y)
    
    # Optimized ResNet (новая версия)
