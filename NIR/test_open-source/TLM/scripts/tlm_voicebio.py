...
# Пути
EMB_DIR = "/home/ext-ivanova-mk@ad.speechpro.com/test_dir/tlm/feature_extraction/voicebio_titanet_embeddings"
MODEL_PATH = "/home/ext-ivanova-mk@ad.speechpro.com/test_dir/tlm/models/tlm_age_mixup_3.pkl"
OUTPUT_DIR = EMB_DIR

print("\n1. Загрузка эмбеддингов...")
all_emb = np.load(os.path.join(EMB_DIR, "all_embeddings_voicebio.npy"))
print(f"   Эмбеддинги загружены, форма: {all_emb.shape}")

if len(all_emb.shape) == 3 and all_emb.shape[1] == 1:
    all_emb = all_emb.squeeze(1)
    print(f"   Размерность скорректирована: {all_emb.shape}")

print("\n2. Загрузка метаданных...")
df_meta = pd.read_csv(os.path.join(EMB_DIR, "voicebio_titanet_metadata.csv"))
...
output_csv = os.path.join(OUTPUT_DIR, "voicebio_titanet_metadata_with_predictions.csv")
df_meta.to_csv(output_csv, index=False)
print(f"   Результат сохранён в {output_csv}")

np.save(os.path.join(OUTPUT_DIR, "age_predictions_voicebio.npy"), predictions)
print(f"   Предсказания сохранены в age_predictions_voicebio.npy")

# Сохраняем файл
output_txt = os.path.join(OUTPUT_DIR, "voicebio_age_predictions.txt")
with open(output_txt, 'w', encoding='utf-8') as f:
    for idx, row in df_meta.iterrows():
        f.write(f"{row['file_path']} {row['age_pred']:.2f}\n")
